...
const photoSlice = createSlice({
  ...
  reducers: {
    ...
    removePhoto: (state, action: PayloadAction<number>) => {
      return state.filter((photo: IPhoto) => photo.id !== action.payload);
    },
    editPhoto: (state, action: PayloadAction<IPhoto>) => {
      const newPhoto = action.payload;
      const photoIndex = state.findIndex((photo: IPhoto) => photo.id === newPhoto.id);
      if (photoIndex >= 0) {
